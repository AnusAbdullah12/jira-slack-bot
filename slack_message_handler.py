...
from jira_helper import get_sprint_issues
from openai_helper import categorize_and_summarize_issues
from release_notes_generator import format_release_notes
from confluence_helper import post_to_confluence
...
def extract_sprint_id(sprint_url):
    """Extracts Sprint ID from a Jira Sprint URL."""
    match = re.search(r"sprint/(\d+)", sprint_url)
    return match.group(1) if match else None


def _respond_generate_release_notes(channel, text):
    """Handles the `/generate-release-notes` command with Sprint URL."""
    sprint_url = text.split(" ")[1] if len(text.split(" ")) > 1 else None
    if sprint_url:
        logging.info(f"Generating release notes for sprint: {sprint_url}")

        sprint_id = extract_sprint_id(sprint_url)
        if not sprint_id:
            send_slack_message(channel, "⚠️ Invalid Sprint URL! Could not extract Sprint ID.")
            return

        issues = get_sprint_issues(sprint_id)
        categorized_issues = categorize_and_summarize_issues(issues)
        formatted_notes = format_release_notes(categorized_issues)
        confluence_url = post_to_confluence(formatted_notes)

        send_slack_message(channel, f"📢 Release Notes Published: https://bylka.atlassian.net/wiki{confluence_url}")
    else:
        send_slack_message(channel, "⚠️ Please provide a valid Sprint URL!")
...
def handle_message(event_data):
    ...
    elif text.startswith("generate-release-notes"):
        _respond_generate_release_notes(channel, text)
